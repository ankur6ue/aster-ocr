...
class ModelBuilder(nn.Module):
  ...
  def __init__(self, dataset_info, args):
    ...
    self.arch = args.arch
    self.rec_num_classes = dataset_info.rec_num_classes
    self.sDim = args.decoder_sdim
    self.attDim = args.attDim
    self.max_len_labels = args.max_len
    self.eos = dataset_info.char2id[dataset_info.EOS]
    self.STN_ON = args.STN_ON
    self.tps_inputsize = args.tps_inputsize
    ...
    if self.STN_ON:
      self.tps = TPSSpatialTransformer(
        output_image_size=tuple(args.tps_outputsize),
        num_control_points=args.num_control_points,
        margins=tuple(args.tps_margins))
      self.stn_head = STNHead(
        in_planes=3,
        num_ctrlpoints=args.num_control_points,
        activation=args.stn_activation)
  ...
  def forward(self, input_dict):
    ...
    if self.STN_ON:
      ...
      # draw_fiducuals(stn_input, ctrl_points)
    ...
    return return_dict, stn_input, x, ctrl_points